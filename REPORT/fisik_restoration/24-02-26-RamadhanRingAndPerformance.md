...
## 1. Context
The objective was to fully integrate the new **Research** and **Ramadhan Ring** landing pages into the React architecture, ensuring full localization (ID/EN) triggered by URL parameters, robust payment processing, and top-tier loading performance via lazy loading. Later in the session, we also focused on CAPI pixel updates and setting critical developer guidelines.
...
- **Checkout & Trust:**
    ...
    - Added 10 international verified reviews with flags (ID, SG, MY, BN, AE, JP, US, AU).
    - Integrated FAQ with visual proof: `testpen.jpeg` (Tester) and `certi.jpeg` (Certificate).
- **CAPI Integration:**
    - Updated Pixel ID to `874165095242407` across relevant pages (Ramadhan Ring, Jewelry).
...
### E. Developer Guidelines Update
- **Model Mandate:** Updated `README.md` and Global Memory to strictly enforce the use of **Gemini 3 Pro** via antigravity or CLI API to prevent lag and model restrictions from Gemini 3 Flash.
...
| Redundant CAPI Tracking | Frontend firing Purchase events unnecessarily. | Stripped frontend Purchase/InitiateCheckout, left backend reminders. |
